  ...
  validator.message['date'] = 'no es una cita real';
  	...
  	$('form').submit(function(e) {
	    ...
	    if (!validator.checkAll($(this)))
	    	submit = false;
	    if (submit){
	    	$("#codigo_superior").val($("#unidad_id option:selected").html());
	      	this.submit();
	    }
	    return false;
  	});
	...
	$('#viewchekeadorcodigincodeofited').on('click',function(){
	...
	$('#unidad_id').change(function(){
      	$.get("/pedircodigodeoficinadeacuerdounidad/"+($(this).val())+"",function(response){

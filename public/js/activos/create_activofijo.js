$(document).ready(function(){
  var sel = $("#fecha_incorporacion_activofijo");
  ...
  sel.change(function(data){
    var jqThis = $(this);
    if(((parseFloat((jqThis.val()).substring(0,4))) == (($("#gestionactualizadetead_activosfijossistemaactualizatedformhhh").html())))&&((jqThis.val())<=($("#fechaactual_activofijo").val()))){
      $.get("../onecotizaciontipocamvioufvstor/"+(jqThis.val()),function(response){
        if(response.success)
  ...
  if(($(this).val())!=0){
    $.get("../getencargforoficeopencounts/"+($(this).val()), function(data){
      if (data.success){
        ...
        showErrorMessage("NO SE ENCONTRO FUNCIONARIOS DE ACUERDO A LA OFICINA SELECCIONADA!");
        $("#responsable_id").empty();
        $("#responsable_id").append("<option value> </option>");
        $("#cargoencargado_activofijo").val('');
      }

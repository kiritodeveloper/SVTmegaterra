$(document).ready(function(){
	...
	$(document).on('keydown','.lottrart',function(e){
		if (e.which == 13){
			...
			var rows = ($("#reg_notaentrada > tbody").children().length)-1;
			if(rows>row){
	...
	function actualisar_totales(){
		...
		var $sum=0;
		var $sume=0;
		$("#datos_aticuloentrada").find('tr').each(function() {
			...
			if(confirm("Desea rellenar el formulario de acuerdo al pedido (numero de pedido insertado en el campo #pedido)?")){
				...
				$.get("../searchformulariopedidofillingreso/"+($(this).val()), function(respuestaget){
					if (respuestaget.success){
						...
						$(".element").val('');
						$(".lottrart").val('');
						$('#datos_aticuloentrada tr:not(:first)').remove();
						...
						showErrorMessage("NO SE ENCONTRO PEDIDO DE MATERIAL O YA SE REGISTRO NOTA DE ENTRADA DE ACUERDO AL NUMERO DE PEDIDO!");
				});
			}
		}
	});
	...
	$(document).on('keyup','.cantmaterrartl',function(e){
		...
		var $suma=0;
		$("#reg_notaentrada tbody input[for=cant_artimat]").each(function(){
			if(($(this).val()) != ""){
	...
	$(document).on('keyup','.impttl',function(e){
    	...
    	var $sum=0;
		$("#reg_notaentrada tbody input[for=totalprearti]").each(function(){
			if(($(this).val()) != ""){
	...
	$(document).on('keyup','.descuentotalart',function(e){
    	...
    	var $sum=0;
		$("#reg_notaentrada tbody input[for=netoimprati]").each(function(){
			if(($(this).val()) != ""){
				$sum=$sum+parseFloat($(this).val());
				$sum = Number(Math.round($sum+'e2')+'e-2');
			}
	...
	$('#pills').bootstrapWizard({
        ...
        onNext: function(tab, navigation, index) {
            if((($("#deposito_id").val())!=null) && (($("#clasenota_id").val())!=null) && (($("#institucion_id").val())!=null)){
            	$("#insticucioncabeceranotaentradaextaparicionnameforapelidad").html($("#institucion_id option:selected").html());
            	$("#almacencabespannotacabeceranametext").html($("#deposito_id option:selected").html());
            	$("#clasnotatrnsaciontitlenotatextettipinoteclasing").html("NOTA DE ENTRADA - "+($("#clasenota_id option:selected").html()));
                return true;
		    }else{
    ...
    $("#notaentradastorefail").keypress(function(e){
        if (e.which == 13)
        	return false;
    });

    $("#datos_aticuloentrada").keypress(function(e){
        if (e.which == 13)
            return false;
    });

    $("#notaentradastorefail").submit(function(e){
    	e.preventDefault();
		...
		unpload_notanetracreate();
		return false;
	});
});

function unpload_notanetracreate(){
	var token = $("#token").val();
	$.ajax({
	  headers: {'X-CSRF-TOKEN': token},
      url: '../file',
      data:new FormData($("#notaentradastorefail")[0]),
      dataType:'json',
      type:'post',
      processData: false,
      contentType: false,
...
function guardar_articuls(cadena){
	var route = $('#notaentradastorefail');
	...
	$.ajax({
		...
		success:function(asient){
			if(asient.success) {
				showSuccess("NOTA DE ENTRADA REGISTRADO SATISFACTORIAMENTE....");
			    document.location.href = '/almacen/notaentrada';
			}else
				showErrorMessage('Error al registrar articulos de nota de entrada!!!');
		},
		error: function() {
	       	showErrorMessage('Error al registrar articulos de nota de entrada!!!');
		}
